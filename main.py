...
class SSHConfiguration:
    def __init__(self, config_file):
        ...
        if not self._servers:
            ...
            print(f"Number of servers in configuration: {num_servers}")            
...
def select_server(config):
    if len(config.servers) > 1:
        print("Multiple servers found. Please choose the server to connect:")
        for i, server in enumerate(config.servers):
            print(f"[{i+1}] {server['name']} ({server['host']}:{server['port']})")

        choice = input("Enter the server number: ")
        try:
            choice = int(choice)
        ...
        print("No servers found in the configuration. Exiting.")
...
def main():
    print("PythonSSH Lab Control - Version 0.1")
    print("+----------------------------------+")
